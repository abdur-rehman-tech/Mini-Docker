...
class Same extends format_1.Format {
                                ...
                                : typeof a !== 'object' || typeof b !== 'object'
                                    ...
                                    : !this.isError() && b instanceof Error
                                        ...
                                        : this.isError() &&
                                            ...
                                            ? false
                                            : this.isSet() && !new format_1.Format(b).isSet()
                                                ? false
                                                : this.isMap() && !new format_1.Format(b).isMap()
                                                    ? false
                                                    : this.isArray() && !new format_1.Format(b).isArray()
                                                        ? false
    ...
    simplePrint(obj, options = {}) {
        return new format_1.Format(obj, {
            ...this.options,
            ...options,
        }).print();
    }
    simplePrintExpect() {
        return new format_1.Format(this.expect, {
            ...this.options,
    ...
    print() {
        if (this.memo === null && this.memoExpect === null) {
            ...
            if (!this.simple) {
                this.unmatch();
                this.memo = (this.memo ?? '') + this.simplePrint(this.object);
                this.memoExpect = (this.memoExpect ?? '') + this.simplePrintExpect();
            }
            else {
    ...
    printCircular(seen) {
        this.memo = (this.memo ?? '') + this.style.circular(seen);
        const seenExpect = this.seenExpect();
        this.memoExpect = this.memoExpect || '';
        if (seenExpect) {
            this.memoExpect = (this.memoExpect ?? '') + this.style.circular(seenExpect);
        }
    }
    diff() {
        // impossible
    ...
    get expectAsArray() {
        const value = Array.isArray(this.expect)
            ? this.expect
            : new format_1.Format(this.expect).isArray()
    ...
    printEnd() {
        ...
        this.memo = (this.memo ?? '') + end;
        this.memoExpect = (this.memoExpect ?? '') + end;
    }
    printPojo() {
    ...
    printPojoEntry(key, val, notFound) {
        ...
        child.print();
        if (!notFound) {
            this.memo = (this.memo ?? '') + child.memo;
        }
        if (notFound || hasOwnProperty.call(this.expect, key)) {
            this.memoExpect = (this.memoExpect ?? '') + child.memoExpect;
        }
    }
    ...
    printErrorTail() {
        ...
        this.memo = (this.memo ?? '') + t;
        this.memoExpect = (this.memoExpect ?? '') + t;
    }
    ...
    printMapEntryUnexpected(key, val) {
        this.unmatch();
        this.memo = (this.memo ?? '') + this.simplePrint(val, {
            key,
            parent: this,
        });
    }
