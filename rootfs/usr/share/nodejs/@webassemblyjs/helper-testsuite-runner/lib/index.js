...
function run(filename) {
  ...
  (0, _child_process.execSync)("wast2json --debug-names ".concat(filename, " -o ").concat(manifestOut));
...
function loadModule(type, filename) {
  ...
  if (type === "text") {
    var content = (0, _fs.readFileSync)((0, _path.join)(WASM_TEST_DIR, filename), "utf8");

    // we need a module in order to be compiled
    var ast = (0, _wastParser.parse)("(module " + content + ")");
