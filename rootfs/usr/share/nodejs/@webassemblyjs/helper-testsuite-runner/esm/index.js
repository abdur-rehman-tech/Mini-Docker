...
export default function run(filename) {
  ...
  execSync("wast2json --debug-names ".concat(filename, " -o ").concat(manifestOut));
...
function loadModule(type, filename) {
  ...
  if (type === "text") {
    var content = readFileSync(join(WASM_TEST_DIR, filename), "utf8");

    // we need a module in order to be compiled
    var ast = parse("(module " + content + ")");
