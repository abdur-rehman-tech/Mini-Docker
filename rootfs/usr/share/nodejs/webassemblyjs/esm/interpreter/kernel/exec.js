...
export function executeStackFrame(_ref, offset, firstFrame) {
  var program = _ref.program;
  if (!(_typeof(program) === "object")) {
    throw new RuntimeError('typeof program === "object"' + " error: " + (undefined || "unknown"));
  }
  ...
  function setLocalByIndex(frame, index, value) {
    if (!(typeof index === "number")) {
      throw new RuntimeError('typeof index === "number"' + " error: " + (undefined || "unknown"));
    }
  ...
  function popArrayOfValTypes(frame, types) {
    if (frame.values.length < types.length) {
      throw new RuntimeError("Assertion error: expected " + JSON.stringify(types.length) + " on the stack, found " + frame.values.length);
    }
  ...
  function pop2(frame, type1, type2) {
    if (frame.values.length < 2) {
      throw new RuntimeError("Assertion error: expected " + JSON.stringify(2) + " on the stack, found " + frame.values.length);
    }
  ...
  function getActiveStackFrame() {
    if (!(framepointer > -1)) {
      throw new RuntimeError('framepointer > -1' + " error: " + ("call stack underflow" || "unknown"));
    }
  ...
  while (true) {
      ...
      case "InternalCallExtern":
        {
          ...
          if (!subroutine.isExternal) {
            throw new RuntimeError('subroutine.isExternal' + " error: " + (undefined || "unknown"));
          }
          var _res2 = subroutine.code(args.map(function (arg) {
            return arg.value;
          }));
          if (typeof _res2 !== "undefined") {
            pushResult(frame, castIntoStackLocalOfType(resultType, _res2));
          }
          break;
        }
    }
    switch (instruction.id) {
      ...
      case "call":
        {
          ...
          // $FlowIgnore
          var _index = instruction.index.value;
          ...
          pc = offsets.indexOf(String(_index));
          break;
        }
      case "end":
        {
          ...
          var found = false;
          var _index = frame.values.slice(0).reverse().findIndex(function (_ref2) {
            var type = _ref2.type;
            return type === "label";
          });
          ...
          if (_index !== -1) {
            var initialOrderIndex = frame.values.length - 1 - _index;
      ...
      case "br_if":
        {
          // $FlowIgnore
          var _label2 = instruction.args[0];
      ...
      case "get_local":
        {
          ...
          if (index.type === "NumberLiteral" || index.type === "FloatLiteral") {
            ...
            throw newRuntimeError("get_local: unsupported index of type: " + index.type);
          }
          break;
        }
      case "set_local":
      ...
      case "set_global":
        {
          ...
          // $FlowIgnore
          var _index4 = instruction.args[0];
      ...
      case "get_global":
        {
          ...
          // $FlowIgnore
          var _index5 = instruction.args[0];
      ...
      case "store32":
        {
          ...
          var memoryBuffer = new Uint8Array(memory.buffer);
      ...
      case "load32_u":
        {
          ...
          var _ptr = pop1OfType(frame, "i32").value.toNumber() + getMemoryOffset(frame, instruction);
      ...
      case "and":
        {
          ...
          switch (instruction.object) {
            ...
            case "f64":
              ...
              break;
            default:
              throw createTrap("Unsupported operation " + instruction.id + " on " +
              // $FlowIgnore
              instruction.object);
          }
      ...
      case "reinterpret/f64":
        {
          ...
          var opType = instruction.id.indexOf("/") !== -1 ?
          // $FlowIgnore
